...
## New in v0.7.0 (Divi 5.9.0 / 5.10.0) — ⚙ source-verified on live 5.10.0

> Extracted directly from a live Divi 5.10.0 install (module.json files + server PHP under `includes/builder-5`), not yet render-tested from raw REST markup:
>
> - **`divi/post-filter` + `divi/post-filter-item`** (5.10) — full content schema, targetLoop/applyMode/relation, field types, comparison operators, URL param namespace (`loop_filter-{loopId}`). See MODULES-DYNAMIC.
> - **`gridOffsetRules`** (5.9 Grid Editor) — container-side child placement, nth-of-type targeting, columnSpan→`grid-column-end: span N` mapping verified in `LayoutStyle.php`. See LAYOUT §5b.
> - **Blurb `alignSelf` alignment** (5.10) — `align_self_to_alignment` mapping verified in `BlurbModule.php`. See MODULES-CONTENT.
> - **Breakpoint contract corrected** — max-width bands from `Framework/Breakpoint/Breakpoint.php`: phone 767 / phoneWide 860 / tablet 980 / tabletWide 1024 (max-width), widescreen 1280 / ultraWide 1440 (min-width); phoneWide+tabletWide+widescreen+ultraWide **disabled by default**. Pre-v0.7.0 phoneWide "480–767px" was wrong. See BASE §6.
> - `builderVersion` → `"5.10.0"`. 5.9/5.10 also shipped automatic below-the-fold lazy loading for videos/sliders/maps (no markup change) and a large batch of migration/Woo/TB fixes (no authoring-schema impact).
...
*DIVI5 Coverage Map — V0.8.2 | Builder Version 5.10.0*
